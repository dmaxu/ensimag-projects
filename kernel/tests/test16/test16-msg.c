...
int test16_msg(void *arg)
{
        ...
        for (count = 1; count <= 100; count++) {
                ...
                p->count = count;
                p->fid = fid;
                pid = start(proc16_1, 2000, 128,"proc16_1", (void*)0);
                assert(pid > 0);
                for (i=0; i<=count; i++) {
        ...
        for (i = 0; i< NB_PROCS; i++) {
                pid = start(proc16_2, 2000, 127,"proc16_2", (void*)0);
                assert(pid > 0);
                pids[i] = pid;
        }
        for (i=0; i < NB_PROCS; i++) {
                pid = start(proc16_3, 2000, 127,"proc16_3", (void*)0);
                assert(pid > 0);
                pids[NB_PROCS + i] = pid;
        }
