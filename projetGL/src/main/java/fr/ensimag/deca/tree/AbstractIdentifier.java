...
public abstract class AbstractIdentifier extends AbstractLValue {
    ...
    protected ListExpr getTailleExpr(DecacCompiler compiler, EnvironmentExp localEnv,
        ...
        ExpDefinition exprDefinition = localEnv.get(getName());
    ...
    protected void setDynamicType(Type type, EnvironmentExp localEnv){
        // On retourne la définition de l'expression (variable, paramétre...)
        ExpDefinition expDefinition = localEnv.get(getName());
        // On preserve le type dynamique
        expDefinition.setDynamicType(type);
    }

    protected Type getDynamicType(EnvironmentExp localEnv){
        // On retourne la définition de l'expression (variable, paramétre...)
        ExpDefinition expDefinition = localEnv.get(getName());
        // On retourne le type dynamique
        return expDefinition.getDynamicType();
    }
}
