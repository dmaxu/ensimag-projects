...
// Parcours qui prend en consideration le fait de supprimer
#define queue_for_each_safe(pos, n, head, type, field)                      \
    for (pos = queue_entry((head)->next, type, field),                     \
         n = queue_entry(pos->field.next, type, field);                    \
         &pos->field != (head);                                            \
         pos = n, n = queue_entry(n->field.next, type, field))

		 
/* ------------------------------------------------------------------ */
/*  Test: un élément est‑il dans une liste chainée ?                  */
/*  head  : tête de la liste                                          */
/*  ptr   : pointeur vers l’élément recherché                         */
/*  type  : type de l’élément                                         */
/*  field : nom du champ link dans la structure                       */
/* ------------------------------------------------------------------ */

#define queue_member(ptr, head, type, field)                             \
    ({ int _found = 0;                                                   \
       type *_qm;                                                        \
       queue_for_each(_qm, head, type, field) {                          \
           if (_qm == (ptr)) { _found = 1; break; }                      \
       }                                                                 \
       _found; })
