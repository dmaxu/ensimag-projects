...
int main(int argc, char *argv[])
{
   ...
   perfstart(&p) ;
#endif
   // long res = NW_cache_oblivious(seq[0], length[0], seq[1], length[1],5) ; 
   // long res = NW_itera(seq[0], length[0], seq[1], length[1]) ;
   long res = NW_itera_cache_aware(seq[0], length[0], seq[1], length[1], 55) ;
#ifdef __PERF_MESURE__
